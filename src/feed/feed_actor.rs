use barter_data::{
    exchange::{binance::spot::BinanceSpotTestnet, ExchangeId},
    streams::Streams,
...
async fn main() {
    ...
    let mut streams = Streams::<OrderBooksL1>::builder()
        ...
        .subscribe([
            (BinanceSpotTestnet::default(), "btc", "usdt", InstrumentKind::Spot, OrderBooksL1),
        ])
    ...
    while let Some(order_book_l1) = binance_stream.recv().await {
        println!("MarketEvent<OrderBookL1>: {order_book_l1:?}");
    }
}
